...
@test "shiftfsMgr basic" {
  ...
  run sh -c 'findmnt | grep shiftfs | grep "/lib/modules/${kernel_rel}" | awk "{ print \$2\":\"\$3\":\"\$4 }"'
  [ "$status" -eq 0 ]
  [[ "$output" =~ "/var/lib/sysbox/shiftfs/".+"/lib/modules/${kernel_rel}".+"shiftfs" ]]

  run sh -c 'findmnt | grep shiftfs | grep "/usr/src/linux-headers-${kernel_rel}" | awk "{ print \$2\":\"\$3\":\"\$4 }"'
  [ "$status" -eq 0 ]
  [[ "$output" =~ "/var/lib/sysbox/shiftfs/".+"/usr/src/linux-headers-${kernel_rel}".+"shiftfs" ]]

  run sh -c 'findmnt | grep shiftfs | grep "/var/lib/docker/containers" | awk "{ print \$2\":\"\$3\":\"\$4 }"'
  [ "$status" -eq 0 ]

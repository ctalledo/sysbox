...
function copy_conf_to_host() {
	cp "$sysbox_artifacts/config/50-sysbox-sysctl.conf" "$host_lib_sysctl/50-sysbox-sysctl.conf"
	cp "$sysbox_artifacts/config/50-sysbox-mod.conf" "$host_usr_lib_mod/50-sysbox-mod.conf"
}

function rm_conf_from_host() {
	rm -f "$host_lib_sysctl/50-sysbox-sysctl.conf"
...
function apply_conf() {
	...
	sysctl -p "$host_lib_sysctl/50-sysbox-sysctl.conf"
